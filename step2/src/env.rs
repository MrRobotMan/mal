...
pub(crate) fn add(ast: &[Token]) -> MalRes<Token> {
    ...
    match [&ast[0], &ast[1]] {
        [Token::Real(n1), Token::Real(n2)] => Ok(Token::Real(n1 + n2)),
        [Token::Integer(n1), Token::Integer(n2)] => Ok(Token::Integer(n1 + n2)),
        [t1, t2] => {
            Err(FnError::Type("Add".into(), t1.token_type().into(), t2.token_type().into()).into())
        }
    }
}
...
pub(crate) fn div(ast: &[Token]) -> MalRes<Token> {
    ...
    match [&ast[0], &ast[1]] {
        [Token::Real(n1), Token::Real(n2)] => {
            if n2 == &0.0 {
                Err(FnError::ZeroDivision.into())
            } else {
                Ok(Token::Real(n1 / n2))
            }
        }
